...
class Trader:
    ...
    def trade_vouchers(
        ...
        for strike in self.VOUCHER_STRIKES:
            ...
            if otm_amount > 20 and best_bid > 0 and pos > -self.VOUCHER_LIMIT:
                base_qty = 5 if strike == 5300 else 10
                qty = min(base_qty, self.VOUCHER_LIMIT + pos)
                if qty > 0:
            ...
            # Cover quickly if spot rallies back toward the strike.
            if otm_amount < 12 and pos < 0 and best_ask is not None:
                qty = min(20, abs(pos))
                if qty > 0:
                    orders[symbol].append(Order(symbol, best_ask, qty))
                    logger.print(
                        f"{symbol} cover quote={best_ask} qty={qty} spot={spot:.1f} otm={otm_amount:.1f}"
                    )
